...
A small window opens. Click **Browse…**, choose the folder that contains your `.pub` files, then click **Convert**. Every `.pub` file in that folder and its subfolders is converted. PDFs are saved beside each source file by default.
...
## Advanced: command-line / batch file
...
| Switch | Purpose |
|--------|---------|
| `-Skip` | Skip files that already have a PDF (default in the GUI and `.bat` file) |
